...
function AdicionarReceita({ onAdicionado }) {
    ...
    async function handleSubmit(e) {
        ...
        setCarregar(true);
        try {
            await api.post('/receitas', { nome, valor: parseFloat(valor) });
            setNome('');
            setValor('');
            onAdicionado();
        } catch(err) {
            setErro(err.response?.data?.mensagem || 'Erro ao adicionar receita.');
        } finally {
            setCarregar(false);
        }
    }
    ...
    return (
        <div>
            <h3 className="text-2xl font-bold text-center mb-6">Adicionar Receita</h3>
